...
\[
C_v = \frac{1}{k_bT}\big(\langle E^2 \rangle - \langle E \rangle^2\big)
\]
where
\begin{align*}
...
\[
\mathcal{M} = \sum_{i=1}^M s
\]
we quicly see that there are only 5 possible values for the magnetization, $\mathcal{M} \in \{-4, -2, 0, 2, 4\}$. The information on multiplicity for a given energy and magnetization is given in table \ref{tab: multiplic}.
...
( abs val table? in \ref{tab: multiplic2})
\\
\\
\begin{tabular}{|c|c|c|}
\hline 
abs Magnetization & Energy &  Multiplicy\\ 
\hline 
4 & -8J & 2 \\ 
\hline 
2 & 0 & 8 \\ 
...
The expectation value of the magetization is then:
\begin{align*}
\langle |\mathcal{M}| \rangle
\end{align*}
absolute value???

We then want to calculate the suceptibility
\[
\chi = \frac{1}{k_bT}\big( \langle \mathcal{M}^2 \rangle - \langle \mathcal{M} \rangle^2 \big)
\]
where 
\begin{align*}
\langle \mathcal{M}^2 \rangle &= \sum_{i=1}^M \mathcal{M}^2 \frac{1}{Z}e^{-\beta E_i} = \frac{1}{Z}\big( 2\cdot 4^2 e^{-\beta (-8J)} + 8\cdot 2^2e^0 + 0 + 0 \big) = \frac{32}{Z}\big( e^{8J\beta} + 1 \big) \\
...
\chi = \frac{1}{k_bT}\big( \langle \mathcal{M}^2 \rangle - \langle \mathcal{M} \rangle^2 \big) = 
\end{align*}
wat? abs value or not?

\subsection{Units, scaled unitless parameters}
Introducing: 
\begin{align*}
T' = T \frac{k_b}{J} \Rightarrow T = T'\frac{J}{k_b}\\
\beta = \frac{1}{k_bT} = \frac{1}{k_b\cdot T'\frac{J}{k_b}} = \frac{1}{T'J}
\end{align*}
...
where $T'$ is the unitless parameter that will be used in the calculations (do with $C_v'$ also and so on ??)
